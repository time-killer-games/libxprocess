...
if [ $(uname) = "Darwin" ]; then
  ...
  export PKG_CONFIG_PATH=/usr/lib/64/pkgconfig && g++ -c gamemaker.cpp xprocess.cpp apiprocess/process.cpp -I. -shared -std=c++17 -static-libgcc -lproc -lkvm -lc -lpthread -DPROCESS_GUIWINDOW_IMPL `pkg-config x11 --cflags --libs` `pkg-config tcl --cflags --libs --static` -fPIC && g++ gamemaker.o process.o xprocess.o -o libxprocess.so -I. -shared -std=c++17 -lproc -lkvm -lc -lpthread -DPROCESS_GUIWINDOW_IMPL `pkg-config x11 --cflags --libs` `pkg-config tcl --cflags --libs --static` -fPIC;
  ar rc libxprocess.a gamemaker.o process.o xprocess.o && rm -f "gamemaker.o" "cproc.o" "xproc.o";
else
